...
from .permutations import get_inv_perm, make_jigsaw_perm
...
class JigsawView(PermuteView):
    ...
    def make_frame(self, im, t, canvas_size=384, knot_seed=0):
        ...
        pieces = [
            p.rotate(90 * (i % 4), resample=Image.BILINEAR, expand=1)
        ...
        base_start_locs = np.stack(
            [corner_start_loc, inner_start_loc, edge_e_start_loc, edge_f_start_loc]
        )
        ...
        start_locs = einsum(
            base_start_locs, rot_mats, "start i, rot j i -> start rot j"
        )
        ...
        for edge_idx, to_swap in enumerate(self.edge_swaps):
            if to_swap:
                ...
                swap_perm[8 + edge_idx], swap_perm[12 + edge_idx] = (
                    swap_perm[12 + edge_idx],
                    swap_perm[8 + edge_idx],
                )
        ...
        avg_locs = (start_locs[:, :2] + end_locs[:, :2]) / 2.0
