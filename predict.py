...
DEEPFLOYD_IF_URL = "https://weights.replicate.delivery/default/deep-floyd/DeepFloyd--IF-I-M-v1.0.tar"
...
class Predictor(BasePredictor):
    ...
    def predict(
            ...
            views: str = Input(description="Comma-separated list of views, e.g. `jigsaw`. Must be same length as prompts"),
            ...
            video: bool = Input(default=True)) -> Iterator[Path]:
        """Run a single prediction on the model"""
        prompts = [prompt.strip() for prompt in prompts.split(',')]
        views = [view.strip() for view in views.split(',')]
        prompt_embeds = [self.stage_1.encode_prompt(f'{style} {p}'.strip()) for p in prompts]
        ...
        if video and len(views) != 2 and views[0] != 'identity':
             print("WARNING: Outputting a video requires only two views, and the first view must be the identity. This run will not output a video.")
             video = False
        for i in range(num_samples):
            sample_dir = save_dir / f'{i:04}'
            sample_dir.mkdir(exist_ok=True, parents=True)
            image = sample_stage_1(self.stage_1, 
                                   prompt_embeds,
            ...
            image = sample_stage_2(self.stage_2,
                                   image,
                                   prompt_embeds,
                                   negative_prompt_embeds, 
                                   views,
